      ...
      {cart.length === 0 ? (
        ...
        <Paper elevation={0} sx={{ p: { xs: 2, md: 4 }, borderRadius: 4 }}>
            ...
            {cart.map(item => (
              <React.Fragment key={item.id}>
                ...
                >
                  ...
                  <ListItemText primary={item.name} secondary={typeof item.price === 'number' ? formatPrice(item.price) : 'Price not available'} />
                </ListItem>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 4, flexWrap: 'wrap', gap: 2 }}>
            <Typography variant="h6">Total: {formatPrice(calculateTotal())}</Typography>
